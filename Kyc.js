...
const Kyc = () => {
  ...
  console.log(srcData._id, 'idddddddd')
    ...
    <>
      <CTable>
        ...
        <CTableBody>
          {
            list?.map((item, id) => {
              const { proofType, number, frontSideProof, backSideProof, selfieProof, address, status, _id } = item

              const sts = status === 0 ? 'Pending' : status === 1 ? 'Verified' : 'Rejected'
